...
for file_path in files:
    ...
    if match:
        ...
        new_hero = hero_template.replace('{title}', title).replace('{desc}', desc)

        # Replace the old header div (usually <div style={{ marginBottom: '30px' }}> ... </div> or similar)
        content = re.sub(r'<div[^>]*margin(?:Bottom)?:\s*\'(?:24|30)px\'[^>]*>.*?<h[12].*?</p>\s*</div>', new_hero, content, flags=re.DOTALL)
