    ...
    <div className="fade-in">
      ...
      {showPeriodModal && (
        <ModalShell
          ...
          footer={
            <>
              <button id="btn-cancel-period" onClick={() => setShowPeriodModal(false)} className="btn" style={{ padding: '10px 20px', border: 'none', color: 'var(--color-text)', cursor: 'pointer', fontWeight: '600', transition: 'all 0.2s' }}>Batal</button>
              <button id="btn-save-period" onClick={savePeriod} disabled={saving} className="siakad-btn-primary" style={{ padding: '10px 24px' }}>
      ...
      {showApplicantDetail && (
        <ModalShell
          ...
          footer={
            <>
              <button onClick={() => setShowApplicantDetail(null)} className="btn" style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: 'var(--glass-bg)', color: 'var(--color-text)', cursor: 'pointer', fontWeight: '600', transition: 'all 0.2s' }}>Tutup</button>
              <button id="btn-detail-verify" onClick={() => updateApplicantStatus(showApplicantDetail.id, 'verified')} disabled={updatingStatus === showApplicantDetail.id} className="btn" style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)', color: '#ffffff', cursor: 'pointer', fontWeight: '600', boxShadow: '0 4px 12px rgba(59, 130, 246, 0.2)', transition: 'all 0.2s' }}>Verifikasi</button>
              <button id="btn-detail-accept" onClick={() => updateApplicantStatus(showApplicantDetail.id, 'accepted')} disabled={updatingStatus === showApplicantDetail.id} className="btn" style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: '#ffffff', cursor: 'pointer', fontWeight: '600', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.2)', transition: 'all 0.2s' }}>Terima</button>
              <button id="btn-detail-reject" onClick={() => updateApplicantStatus(showApplicantDetail.id, 'rejected')} disabled={updatingStatus === showApplicantDetail.id} className="btn" style={{ padding: '10px 18px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: '#ffffff', cursor: 'pointer', fontWeight: '600', boxShadow: '0 4px 12px rgba(239, 68, 68, 0.2)', transition: 'all 0.2s' }}>Tolak</button>
            </>
          }
        >
