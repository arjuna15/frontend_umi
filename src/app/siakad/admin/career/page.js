...
export default function CareerCenterPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (jRes.ok) { const d = await jRes.json(); setJobs(Array.isArray(d.data || d.jobs || []) ? (d.data || d.jobs || []) : []); }
      if (sRes.ok) {
  ...
  const jobBadge = (s) => { const c = s === 'open' ? '#10b981' : s === 'closed' ? '#ef4444' : '#f59e0b'; const l = s === 'open' ? 'Dibuka' : s === 'closed' ? 'Ditutup' : 'Draft'; return <span className="siakad-badge-status" style={{ color: c, borderColor: `${c}33`, minWidth: '110px' }}>{l}</span>; };
  const appBadge = (s) => {
    ...
    const [c, l] = map[s] || ['#94a3b8', s]; return <span className="siakad-badge-status" style={{ color: c, borderColor: `${c}33`, minWidth: '120px' }}>{l}</span>;
  };
  const typeBadge = (t) => { const c = t === 'Full-time' ? '#3b82f6' : t === 'Part-time' ? '#8b5cf6' : t === 'Internship' ? '#14b8a6' : '#f59e0b'; return <span className="siakad-badge-status" style={{ color: c, borderColor: `${c}33`, minWidth: '120px' }}>{t}</span>; };
  ...
  const filteredJobs = jobsList.filter(j => !search || (j.company_name || '').toLowerCase().includes(search.toLowerCase()) || (j.position_title || '').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="fade-in">
      ...
      <div className="siakad-card" style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', padding: '24px', marginBottom: '32px', borderRadius: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <div>
            <p style={{ margin: '0 0 4px 0', color: 'var(--color-muted)', fontSize: '0.75rem', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Statistik Career</p>
            <h3 style={{ fontSize: '1.1rem', fontWeight: '800', color: 'var(--color-text)', margin: 0 }}>Pelamar per Bulan</h3>
          </div>
          <span className="siakad-badge-status" style={{ color: '#8b5cf6', borderColor: 'rgba(139,92,246,0.3)', minWidth: '130px' }}>
            {stats.total_applicants} Pelamar
          </span>
        </div>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px' }}>
              {filteredJobs.length === 0 ? (
                ...
                <div key={j.id} className="siakad-card" style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', borderRadius: '20px', padding: '20px', transition: 'all 0.2s' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <CustomSelect
                ...
                options={[
                  ...
                  ...jobsList.map(j => ({ value: j.id.toString(), label: `${j.position_title} - ${j.company_name}` }))
                ]}
              />
            </div>
      ...
      {showModal && (
        <ModalShell title={editId ? 'Edit Lowongan' : 'Tambah Lowongan'} onClose={() => setShowModal(false)} maxWidth="700px" footer={
          <>
            <button id="btn-cancel-job" onClick={() => setShowModal(false)} style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', color: 'var(--color-text)', padding: '10px 20px',   cursor: 'pointer', fontWeight: '600' }}>Batal</button>
            <button id="btn-save-job" onClick={saveJob} disabled={saving} className="siakad-btn-primary" style={{ background: 'linear-gradient(135deg, #C41E3A 0%, #9b1c2e 100%)', color: 'white', border: 'none', boxShadow: '0 4px 12px rgba(196, 30, 58, 0.25)', padding: '10px 24px' }}>{saving ? 'Menyimpan...' : editId ? 'Perbarui' : 'Tambah'}</button>
          </>
        }>
            ...
            {formFields.map(f => (
              <div key={f.key} style={{ marginBottom: '4px', gridColumn: f.span ? 'span 2' : undefined }}>
                ...
                {f.type === 'select' ? (
                  ...
                  <textarea id={`input-job-${f.key}`} className="siakad-input" value={formData[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} placeholder={f.placeholder || ''} rows={3} style={{ boxShadow: 'inset 4px 4px 8px var(--inset-shadow-dark), inset -4px -4px 8px var(--inset-shadow-light)', background: 'var(--liquid-bg)', border: 'var(--inset-border)', color: 'var(--color-text)', resize: 'vertical' }} />
                ) : (
                  <input id={`input-job-${f.key}`} className="siakad-input" type={f.type || 'text'} value={formData[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} placeholder={f.placeholder || ''}  style={{ boxShadow: 'inset 4px 4px 8px var(--inset-shadow-dark), inset -4px -4px 8px var(--inset-shadow-light)', background: 'var(--liquid-bg)', border: 'var(--inset-border)', color: 'var(--color-text)' }} />
                )}
              </div>
            ))}
          </div>
