...
export default function KerjasamaPage() {
  ...
  const statusBadge = (s) => { const c = s === 'active' ? '#10b981' : s === 'expired' ? '#ef4444' : '#f59e0b'; const l = s === 'active' ? 'Aktif' : s === 'expired' ? 'Berakhir' : 'Draft'; return <span style={{ background: `${c}20`, color: c, padding: '4px 12px', borderRadius: '20px', fontSize: '0.78rem', fontWeight: '600' }}>{l}</span>; };
  const tBadge = (t) => { const map = { industri: '#3b82f6', pemerintah: '#8b5cf6', universitas: '#14b8a6', ngo: '#f59e0b' }; const c = map[t] || '#94a3b8'; return <span style={{ background: `${c}20`, color: c, padding: '4px 12px', borderRadius: '20px', fontSize: '0.78rem', fontWeight: '600' }}>{t?.charAt(0).toUpperCase() + t?.slice(1)}</span>; };
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        <div className="siakad-card" style={{ padding: '24px' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', color: 'var(--color-text)', margin: '0 0 20px 0' }}>Distribusi Tipe Kerjasama</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {typeData.map((d, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '0.82rem', color: 'var(--color-muted)' }}>{d.type}</span>
                  <span style={{ fontSize: '0.82rem', fontWeight: '600', color: 'var(--color-text)', marginLeft: 'auto' }}>{d.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="siakad-card" style={{ padding: '24px' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', color: 'var(--color-text)', margin: '0 0 20px 0' }}>Timeline Kerjasama Aktif</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '200px', overflowY: 'auto' }}>
            {(filtered.filter(p => p.status === 'active').slice(0, 6).length > 0 ? filtered.filter(p => p.status === 'active').slice(0, 6) : [
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--color-muted)', minWidth: '100px', textAlign: 'right' }}>{p.start_date?.slice(0, 7) || '-'}</span>
                <div style={{ flex: 1, height: '24px', background: 'rgba(59,130,246,0.15)', borderRadius: '6px', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '70%', background: 'linear-gradient(90deg, #3b82f6, #8b5cf6)', borderRadius: '6px' }}></div>
                  <span style={{ position: 'absolute', left: '8px', top: '50%', transform: 'translateY(-50%)', fontSize: '0.7rem', fontWeight: '600', color: 'white', whiteSpace: 'nowrap' }}>{p.partner_name}</span>
                </div>
                <span style={{ fontSize: '0.75rem', color: 'var(--color-muted)', minWidth: '100px' }}>{p.end_date?.slice(0, 7) || '-'}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="siakad-card" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <input id="search-kerjasama" type="text" placeholder="Cari mitra atau no. MoU..." value={search} onChange={e => setSearch(e.target.value)} style={{ padding: '10px 16px', fontSize: '0.9rem', border: '1px solid var(--color-border)', borderRadius: '10px', background: 'var(--color-bg)', color: 'var(--color-text)', width: '280px' }} />
          <button id="btn-add-kerjasama" onClick={() => { resetForm(); setShowModal(true); }} className="siakad-btn-primary" style={{ padding: '10px 24px' }}><i className="ph ph-plus"></i> Tambah Kerjasama</button>
        </div>
      ...
      {showModal && (
        <ModalShell title={editId ? 'Edit Kerjasama' : 'Tambah Kerjasama'} onClose={() => setShowModal(false)} footer={<>
          <button id="btn-cancel-ks" onClick={() => setShowModal(false)} style={{ padding: '10px 20px', border: 'none', color: 'var(--color-text)', cursor: 'pointer', fontWeight: '600' }}>Batal</button>
          <button id="btn-save-ks" onClick={savePartnership} disabled={saving} className="siakad-btn-primary" style={{ padding: '10px 24px' }}>{saving ? 'Menyimpan...' : editId ? 'Perbarui' : 'Tambah'}</button>
        </>}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {formFields.map(f => (
              <div key={f.key} style={{ marginBottom: '4px', gridColumn: f.span ? 'span 2' : undefined }}>
                ...
                {f.type === 'select' ? (
                  <select id={`input-ks-${f.key}`} value={formData[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} style={{ width: '100%', padding: '10px 14px', fontSize: '0.9rem', boxSizing: 'border-box', color: 'var(--color-text)' }}>
                    {f.options.map(o => <option key={o} value={o}>{o.charAt(0).toUpperCase() + o.slice(1)}</option>)}
                  </select>
                ) : f.type === 'textarea' ? (
                  <textarea id={`input-ks-${f.key}`} value={formData[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} placeholder={f.placeholder || ''} rows={3} style={{ width: '100%', padding: '10px 14px', fontSize: '0.9rem', boxSizing: 'border-box', color: 'var(--color-text)', resize: 'vertical' }} />
                ) : (
                  <input id={`input-ks-${f.key}`} type={f.type || 'text'} value={formData[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} placeholder={f.placeholder || ''} style={{ width: '100%', padding: '10px 14px', fontSize: '0.9rem', boxSizing: 'border-box', color: 'var(--color-text)' }} />
                )}
              </div>
            ))}
          </div>
