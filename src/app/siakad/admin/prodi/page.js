  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (userRes.ok) {
        ...
        const lecturers = users.filter(u => u.role === 'dosen' || u.role === 'kaprodi');
        setDosens(lecturers.map(l => ({ value: l.name, label: `${l.name} (${l.nim_nip || 'Dosen'})` })));
      }
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="fade-in" style={{ paddingBottom: '40px' }}>
        ...
        <div className="siakad-page-header-grid"></div>
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div style={{ flex: '1 1 300px' }}>
              ...
              <h1 style={{ fontSize: '2.2rem', fontWeight: '800', margin: '0 0 8px 0', letterSpacing: '-0.03em' }}>Manajemen Program Studi</h1>
              <p style={{ margin: 0 }}>Kelola daftar Program Studi dan Ketua Program Studi (Kaprodi).</p>
            </div>
      ...
      <div className="siakad-card" style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', padding: '0px', overflow: 'hidden' }}>
        <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', borderBottom: '1px solid var(--color-border)' }}>
          <h3 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--color-text)', fontWeight: 'bold' }}>Daftar Program Studi</h3>
          <div style={{ position: 'relative', width: '300px' }}>
            ...
            <input type="text" className="siakad-input" placeholder="Cari kode, nama prodi, kaprodi..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ boxShadow: 'inset 4px 4px 8px var(--inset-shadow-dark), inset -4px -4px 8px var(--inset-shadow-light)', background: 'var(--liquid-bg)', border: 'var(--inset-border)', color: 'var(--color-text)', width: '100%', paddingLeft: '46px',  fontSize: '0.9rem' }} />
          </div>
        </div>
              ...
              {(() => {
                const filtered = prodis.filter(p => {
                  ...
                  return (
                    ...
                    p.kaprodi?.toLowerCase().includes(query) ||
                    p.jenjang?.toLowerCase().includes(query)
                  );
                });
                ...
                if (filtered.length === 0) {
                  return (
                    <tr>
                      <td colSpan="5" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-muted)' }}>Tidak ada data program studi.</td>
                    </tr>
                  );
                }
                ...
                return filtered.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid var(--color-border)', transition: 'all 0.2s' }} onMouseEnter={(e)=>e.currentTarget.style.background='var(--glass-bg)'} onMouseLeave={(e)=>e.currentTarget.style.background='transparent'}>
                    ...
                    <td style={{ padding: '16px' }}>
                      <span className="siakad-badge" style={{ color: '#3b82f6' }}>{p.jenjang}</span>
                    </td>
                    <td style={{ padding: '16px', textAlign: 'right' }}>
                      <button onClick={() => { setEditFormData(p); setIsEditModalOpen(true); }} style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', color: '#3b82f6', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginRight: '8px' }}><i className="ph ph-pencil-simple"></i></button>
                      <button onClick={() => handleDelete(p.id)} style={{ background: 'var(--glass-bg)', border: 'var(--glass-border)', boxShadow: 'var(--glass-shadow)', color: '#ef4444', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><i className="ph ph-trash"></i></button>
                    </td>
                  </tr>
                ));
              })()}
        ...
        <ModalShell
          ...
          footer={(
            <>
              ...
              <button type="submit" form="prodi-form" className="siakad-btn-primary" style={{ background: 'linear-gradient(135deg, #C41E3A 0%, #9b1c2e 100%)', color: 'white', border: 'none', boxShadow: '0 4px 12px rgba(196, 30, 58, 0.25)', padding: '12px 20px', fontWeight: 700 }}>Simpan</button>
            </>
          )}
        >
          <form id="prodi-form" onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' , flexWrap: 'wrap'}}>
            <div>
              ...
              <input type="text" required value={editFormData.code} onChange={e=>setEditFormData({...editFormData, code: e.target.value})} className="siakad-input" style={{ boxShadow: 'inset 4px 4px 8px var(--inset-shadow-dark), inset -4px -4px 8px var(--inset-shadow-light)', background: 'var(--liquid-bg)', border: 'var(--inset-border)', color: 'var(--color-text)', width: '100%' }} placeholder="Contoh: TK" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '8px', fontWeight: 600 }}>Nama Prodi</label>
              <input type="text" required value={editFormData.name} onChange={e=>setEditFormData({...editFormData, name: e.target.value})} className="siakad-input" style={{ boxShadow: 'inset 4px 4px 8px var(--inset-shadow-dark), inset -4px -4px 8px var(--inset-shadow-light)', background: 'var(--liquid-bg)', border: 'var(--inset-border)', color: 'var(--color-text)', width: '100%' }} placeholder="Contoh: Teknik Komputer" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '8px', fontWeight: 600 }}>Ketua Program Studi (Kaprodi)</label>
              <CustomSelect
                value={editFormData.kaprodi}
                onChange={val => setEditFormData({...editFormData, kaprodi: val})}
                options={dosens}
