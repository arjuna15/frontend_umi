...
export default function DosenDashboard() {
  ...
  if (loading || !data) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyItems: 'center', height: '100%', color: '#6b7280' }}>
      <i className="ph-spinner ph-spin" style={{ fontSize: '2rem', marginRight: '10px' }}></i> Memuat Dashboard...
    </div>
  );
    ...
    <div className="fade-in" style={{ paddingBottom: '40px' }}>
      <div style={{ marginBottom: '30px' }}>
        <h2 style={{ margin: '0 0 8px 0', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '10px' }}>
          Jadwal & Pengingat Anda <i className="ph-calendar-check" style={{ color: '#3b82f6' }}></i>
        </h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        <div className="siakad-card stagger-1" style={{ padding: '24px' }}>
          <h3 style={{ margin: '0 0 20px 0', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <i className="ph-clock" style={{ color: '#8b5cf6' }}></i> Jadwal Hari Ini
          </h3>
            ...
            {data.schedule && data.schedule.length > 0 ? data.schedule.map((sch, i) => (
              <div key={i} style={{ padding: '16px', background: '#f8fafc', borderRadius: '12px', borderLeft: '4px solid #8b5cf6' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', fontSize: '0.9rem', color: '#6b7280' }}>
                  <span><i className="ph-clock"></i> {sch.time}</span>
                  <span><i className="ph-map-pin"></i> {sch.room}</span>
                  <span><i className="ph-chalkboard"></i> Sesi {sch.meeting}</span>
                </div>
              </div>
            )) : (
        ...
        <div className="siakad-card stagger-2" style={{ padding: '24px' }}>
          <h3 style={{ margin: '0 0 20px 0', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <i className="ph-list-checks" style={{ color: '#10b981' }}></i> To-Do List & Notifikasi
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {data.todos && data.todos.map((todo, i) => (
              <div key={i} style={{ padding: '12px 16px', background: '#ecfdf5', borderRadius: '8px', color: '#065f46', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                <i className="ph-warning-circle" style={{ marginTop: '2px', color: '#10b981' }}></i>
