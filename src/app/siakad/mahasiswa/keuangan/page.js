      ...
      <div className="siakad-card" style={{ padding: '0px', overflow: 'hidden' }}>
        <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <div style={{ position: 'relative', width: '300px' }}>
            ...
            <input type="text" className="siakad-input" placeholder="Cari tagihan, status..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ width: '100%', paddingLeft: '46px', color: 'var(--color-text)', fontSize: '0.9rem' }} />
          </div>
        </div>
                      ...
                      <td style={{ padding: '16px' }}>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {bill.status !== 'Lunas' ? (
                            ...
                            <button onClick={() => window.print()} style={{ padding: '8px 18px', background: '#10b981', color: 'white', border: 'none', borderRadius: '50px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
