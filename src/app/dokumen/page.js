...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch(`${api}/contents`)
      .then(res => res.json())
      .then(data => {
        const htmlContent = data.find(c => c.key === 'dokumen_html');
        const bgContent = data.find(c => c.key === 'dokumen_hero_bg');
      ...
      .catch(err => console.error('Error fetching content for dokumen:', err));
  }, []);
