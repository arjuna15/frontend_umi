...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch(`${api}/contents`)
      .then(res => res.json())
      .then(data => {
        const htmlContent = data.find(c => c.key === 'fasilitas_html');
        const bgContent = data.find(c => c.key === 'fasilitas_hero_bg');
      ...
      .catch(err => console.error('Error fetching content for fasilitas:', err));
  }, []);
    
  
  const heroBg = 'https://umiba.ac.id/wp-content/uploads/2024/05/bannerUMIBA26_1.png';
  const heroTitle = (lang === "en" ? "UMIBA Facilities" : 'Fasilitas UMIBA');
